...
public class ImmutableListAdapter implements JsonDeserializer<List<?>> {
    @Override
    public List<?> deserialize(final JsonElement json, final Type type, final JsonDeserializationContext context) throws JsonParseException {
    ...
    private static <T> TypeToken<List<T>> listTypeOf(final Type entry) {
        return new TypeToken<List<T>>() {}
                .where(new TypeParameter<>() {}, (TypeToken<T>) TypeToken.of(entry));
    }
}
